-- Terra: fotos públicas dos anúncios.
-- Execute depois de 202609060001_terra_accounts_listings.sql.
-- As imagens ficam em um bucket público; upload e exclusão continuam protegidos por RLS.
begin;
...
create policy terra_listing_photos_read
on public.terra_listing_photos
for select to anon, authenticated
using (
  exists (
    select 1
    from public.terra_listings as l
...
create policy terra_listing_photos_insert
on public.terra_listing_photos
...
create policy terra_listing_photos_update
on public.terra_listing_photos
...
create policy terra_listing_photos_delete
on public.terra_listing_photos
...
create policy terra_listing_photos_storage_insert
on storage.objects
for insert to authenticated
with check (
...
create policy terra_listing_photos_storage_delete
on storage.objects
for delete to authenticated
using (
  bucket_id = 'terra-listing-photos'
...
where n.nspname = 'public'
  and c.relname = 'terra_listing_photos';
