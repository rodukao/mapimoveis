window.TerraOperations=(()=>{
 ...
 async function challenge(action,consume){
 ...
 async function contact(body,onChallenge){
  const send=async token=>{const response=await fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json',...(currentSession?.access_token?{Authorization:'Bearer '+currentSession.access_token}:{})},body:JSON.stringify({...body,token})});const result=await response.json();if(!response.ok)throw Error(result.error||'Contato indisponível.');return result;};
  try{return await send();}catch(e){if(!e.message.includes('CAPTCHA_REQUIRED'))throw e;onChallenge?.();return challenge('contact',send);}
 }
 return {withChallenge,invoke,deleteAccount,contact};
})();
