...
(() => {
  ...
  const fields = 'id,owner_id,title,description,city,state,neighborhood,category,price_brl,status,boundary_geojson,area_m2,perimeter_m,latitude,longitude,price_per_m2,revision,created_at,updated_at,terrain_context,topography,infrastructure,features,documents,details,boosted_until';
  ...
  async function list({ mine = false, limit = 50, offset = 0, price = 0, area = 0, sort = 'recent', location = null } = {}) {
    ...
    unwrap(result);
    return { rows: await hydratePhotos(result.data), total: result.count };
  }
