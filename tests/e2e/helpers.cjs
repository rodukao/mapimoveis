...
async function catalog(page){
  await page.goto('/');
  await expect(page.locator('#count')).toContainText('imóveis encontrados');
  await expect(page.locator('#catalog-update')).toBeHidden();
  await expect(page.locator('#cards .card').first()).toBeAttached();
}
async function showList(page){const toggle=page.locator('#mobile-nav-list');if(await toggle.isVisible())await toggle.click();}
...
async function account(page,section){await page.locator('#account:visible, #mobile-nav-account:visible').click();await page.getByRole('dialog',{name:'Minha conta',exact:true}).getByRole('button',{name:section,exact:true}).click();await expect(page.getByRole('dialog',{name:'Minha conta',exact:true})).toBeVisible();}
