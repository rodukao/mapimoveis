...
function setup({row={id:'listing',revision:2},contactError=null}={}){
 ...
 const context={window:{TerraRepository:R},TerraPhotos:{optimize:async file=>file},TerraOperations:{withChallenge:async(action,task)=>{calls.push(['challenge',action]);return task();}},URL};vm.createContext(context);vm.runInContext(fs.readFileSync('dist/js/repository/marketplace.js','utf8'),context);return {api:context.window.TerraMarketData,calls};
}
